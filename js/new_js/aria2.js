...
if (typeof ARIA2 == "undefined" || !ARIA2) var ARIA2 = (function() {
	...
	function default_error(result) {
		...
		$("#main-alert .alert-msg").html("<strong>é”™è¯¯: </strong>"+error_msg);
		$("#main-alert").show();
	}
	...
	function get_title(result) {
		...
		else if (result.files[0].path.replace(new RegExp("^"+dir.replace(/\\/g, "/").replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&')+"/?"), "").split("/").length) {
		...
		if (title.length <= 0) title = "[**æœªæ˜Žç¡®çš„ä¸‹è½½ä»»åŠ¡**]";
		return title;
	}
	...
	return {
		init: function(path, onready) {
			var connect_msg_id = main_alert("alert-info", "æ­£åœ¨è¿žæŽ¥...");
			...
			if (jsonrpc_interface.indexOf("http") == 0) {
				...
				jsonrpc_ws.onerror = function(event) {
					...
					main_alert("alert-error", "WebSocket è¿žæŽ¥é”™è¯¯. ä½ å¯èƒ½éœ€è¦åˆ·æ–°æ­¤é¡µé¢.");
					ws_callback = {};
				};
				...
				main_alert("alert-error", "JSON-RPC Pathé”™è¯¯, åªæ”¯æŒHTTPå’ŒWSåè®®");
			};
		},
		...
		add_task: function(uri, options) {
			...
			ARIA2.request("addUri", [[uri], options],
				...
				function(result) {
					...
					$("#add-task-alert").show();
					// console.warn("æ·»åŠ ä»»åŠ¡é”™è¯¯: "+error_msg);
				}
			);
		},
		...
		tell_active: function(keys) {
			...
			ARIA2.request("tellActive", keys,
				function(result) {
					...
					if (select_lock) return;
					if (!result.result) {
						main_alert("alert-error", "<strong>é”™è¯¯: </strong>RPC é€šä¿¡é”™è¯¯.", 5000);
					}
					...
					if (finished_tasks_list === undefined) {
						...
						$.each(result, function(i, e) {
							...
							if (ARIA2.finish_notification) {
								YAAW.notification("Aria2 Web æŽ§åˆ¶å°é€šçŸ¥", e.title + " å·²å®Œæˆ");
							}
							finished_tasks_list.push(e.gid);
						});
					}
		...
		change_pos: function(gid, pos, how) {
			ARIA2.request("changePosition", [gid, pos, how],
				function(result) {
					// console.debug(result);
					main_alert("alert-info", "ç§»åŠ¨æˆåŠŸ", 1000);
					ARIA2.refresh();
				}
			);
		},
		...
		pause: function(gids) {
			...
			ARIA2.batch_request("pause", gids,
				function(result) {
					...
					if (error.length == 0) {
						main_alert("alert-info", "å·²æš‚åœ", 1000);
						ARIA2.refresh();
					} else {
		...
		unpause: function(gids) {
			...
			ARIA2.batch_request("unpause", gids,
				function(result) {
					...
					if (error.length == 0) {
						main_alert("alert-info", "å·²å¼€å§‹", 1000);
						ARIA2.refresh();
					} else {
						main_alert("alert-error", error.join("<br />"), 3000);
					}
		...
		remove: function(gids) {
			...
			ARIA2.batch_request("remove", gids,
				function(result) {
					...
					if (error.length == 0) {
						main_alert("alert-info", "å·²åˆ é™¤", 1000);
						ARIA2.refresh();
					} else {
						main_alert("alert-error", error.join("<br />"), 3000);
					}
		...
		change_options: function(gid, options) {
			ARIA2.request("changeOption", [gid, options],
				function(result) {
					// console.debug(result);
					main_alert("alert-info", "é€‰é¡¹å·²æ›´æ–°", 1000);
				}
			);
		},
		...
		get_peers: function(gid) {
			ARIA2.request("getPeers", [gid],
				function(result) {
					...
					.append('<tr><th>ä½ç½®</th><th>å®¢æˆ·ç«¯</th><th>å®ŒæˆçŽ‡</th><th>ä¸‹è½½é€Ÿåº¦</th><th>ä¸Šä¼ é€Ÿåº¦</th></tr>')
		...
		pause_all: function() {
			ARIA2.request("pauseAll", [],
				function(result) {
					// console.debug(result);
					ARIA2.refresh();
					main_alert("alert-info", "å·²æš‚åœæ‰€æœ‰ä»»åŠ¡, è¯·ç­‰å¾… BitTorrent æœåŠ¡å™¨å“åº”.", 2000);
				}
			);
		},
		...
		unpause_all: function() {
			ARIA2.request("unpauseAll", [],
				function(result) {
					// console.debug(result);
					ARIA2.refresh();
					main_alert("alert-info", "å–æ¶ˆæš‚åœæ‰€æœ‰ä»»åŠ¡", 2000);
				}
			);
		},
		...
		purge_download_result: function() {
			ARIA2.request("purgeDownloadResult", [],
				function(result) {
					// console.debug(result);
					ARIA2.refresh();
					main_alert("alert-info", "ç§»é™¤æ‰€æœ‰å·²å®Œæˆ/é”™è¯¯/å·²åˆ é™¤ä¸‹è½½ä»»åŠ¡.", 2000);
				}
			);
		},
			...
			ARIA2.request("changeGlobalOption", [options],
				function(result) {
					if (!result.result)
						...
						main_alert("alert-success", "å·²ä¿å­˜", 2000);
				}
			);
		},
			...
			ARIA2.request("getGlobalStat", [],
				function(result) {
					...
					if (result.downloadSpeed > 0) {
						 title += "â†“" + YAAW.tpl.view.format_size_0()(result.downloadSpeed) + "/s ";
					}
					if (result.uploadSpeed > 0) {
						title += "â†‘" + YAAW.tpl.view.format_size_0()(result.uploadSpeed) + "/s ";
					}
					title += (title ? "- " : "") + "Aria2 Web æŽ§åˆ¶å°";
		...
		shutdown: function() {
			...
			if(window.confirm("ç¡®å®šè¦å…³é—­è¿œç¨‹ä¸‹è½½æœåŠ¡å—?")){
				ARIA2.request("shutdown", [],
					function(result) {
						if (!result.result) {
							main_alert("alert-error", "<strong>é”™è¯¯: </strong>RPC é€šä¿¡é”™è¯¯.", 5000);
						} else {
							main_alert("alert-success", "å³å°†å…³é—­", 5000);
						}
					}
				);
			}
...
*/
			...
			ARIA2.request("changeOption", [gid, options],
				function(result) {
					if (!result.result) {
						...
						main_alert("alert-success", "é€‰é¡¹æ›´æ”¹æˆåŠŸ!", 2000);
					}
				}
			);
		},
		...
		auto_refresh: function(interval) {
			...
			interval_id = window.setInterval(function() {
				...
				if (select_lock) {
					if (need_refresh) {
						main_alert("", "ä»»åŠ¡åˆ—è¡¨å·²ç»æ”¹å˜ã€‚ç‚¹å‡»â€œåˆ·æ–°â€æŒ‰é’®æ¥æ›´æ–°ä»»åŠ¡åˆ—è¡¨ã€‚");
					}
				} else {
					if (need_refresh) {
						ARIA2.refresh();
